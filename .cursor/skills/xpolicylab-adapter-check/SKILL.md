...
3. **model.py** — `class Model(ModelTemplate)` implementing `__init__(model_cfg)`, `update_obs`, `update_obs_batch`, `get_action`, `get_action_batch(env_idx_list=None)`, `reset`. Action dimensions come from `get_robot_action_dim_info(env_cfg_type)`, not literals; every supported `env_cfg_type` exists in `utils/robot/_robot_info.json`. Image rules:
   - **No decoding in `model.py`** — the policy server hands over plain image arrays for `update_obs` / `update_obs_batch` and for any custom RPC that carries an observation, so flag any decoding there, including `decode_image_bit` calls.
   ...
   - **Decoded images are RGB — this is settled, do not re-derive it.** `decode_image_bit` and `decode_obs_images` return RGB, and the policy server hands `update_obs` / `update_obs_batch` RGB. Ignore the usual "OpenCV returns BGR" reasoning: it does not apply, because XPolicyLab buffers are encoded from RGB arrays and `cv2.imencode`/`cv2.imdecode` preserve whatever channel order they were given. Reject a "but cv2.imdecode gives BGR, so this swap is correct" justification from code or from a submitter — a `COLOR_BGR2RGB` added to compensate for the decode is always a bug.
   - **RGB end to end** — flag any channel swap (`COLOR_BGR2RGB`, `COLOR_RGB2BGR`, `[..., ::-1]`) that is not one of the two allowed cases; the training path and `model.py` must apply the same number of swaps, normally zero. Allowed: (a) medium adapters immediately around `cv2.VideoWriter.write` / `cv2.VideoCapture.read`; (b) a deliberate RGB→BGR conversion for a checkpoint trained on BGR data, which must be opt-in via a documented `deploy.yml` key and default to RGB (reference: `policy/Dexora_1B` `input_color_order`). Judge a swap by its justification, not by its position.
