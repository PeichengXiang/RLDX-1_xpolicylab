...
# Isaac Sim inserts its bundled (legacy) ``websockets`` directory into the
# interpreter's import search path while SimulationApp starts.  The RLDX
# transport uses the modern asyncio API and the package's lazy exports; if
# these submodules aren't loaded before Isaac starts, the root package from
# the RLDX environment can combine with Isaac's legacy ``websockets.client``
# and fail with a misleading missing ``backoff`` import.  Cache both modern
# modules at the private import boundary so later lazy lookups stay coherent.
