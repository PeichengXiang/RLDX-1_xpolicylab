...
def _extract_video_segment(
    ...
    cmd = [
        ...
        "-i",
        str(src),
        # The v3 episode boundary is half-open.  Cutting stream-copy video by
        # its floating-point duration includes the first packet of the next
        # episode on ffmpeg 7 for this 30 Hz dataset.  Limit the output by the
        # authoritative parquet episode length instead.
        "-frames:v",
        str(expected_frames),
        "-c",
        "copy",
    ...
    for video_key in video_keys:
        ...
        for (chunk_idx, file_idx), records in tqdm.tqdm(
            ...
            for record in records:
                ...
                expected_frames = int(record.get("length", 0))
                if expected_frames <= 0:
